...
\begin{frame}{Q2: Analysis Approach}
    ...
    \begin{tikzpicture}[
        ...
        \node[box, fill=green!20, below right=10pt and 18pt of data] (ineq) {Expenses and Inequality\\(over time)};
        \node[box, fill=blue!20, below=22pt of data] (geo) {Demographic\\(Clustering)};
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.32\textwidth}
            ...
            \footnotesize
            \begin{itemize}\setlength{\itemsep}{1pt}
                \item Demographic, expense and salary features
...
\begin{frame}{Clustering Summary (k = 3)}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            	\textbf{Model Choice}
            \footnotesize
            \begin{itemize}\setlength{\itemsep}{2pt}
                \item Elbow plot used to balance fit vs. complexity
                \item Smallest stable solution selected: \textbf{$k=3$ clusters}
                \item Used all participant data (+ financial journal) 
            \end{itemize}

            \vspace{6pt}
            	\textbf{How It’s Used in the Dashboard}
            \footnotesize
            \begin{itemize}\setlength{\itemsep}{2pt}
                \item \textbf{Cluster} filter applied to the PCP
                \item Same filter applied to the \textbf{Living Gap scatterplot}
                \item Global time slider
            \end{itemize}
        \end{column}
        \begin{column}{0.50\textwidth}
            ...
            \begin{tikzpicture}[
                node distance=8pt,
                cbox/.style={rectangle, draw, rounded corners, minimum width=4.6cm, minimum height=0.85cm, font=\scriptsize, align=center}
            ]
                \node[cbox, fill=red!15, draw=red!60!black] (c1) {\textbf{Affluent Achievers}\\High income, strong buffer};
                \node[cbox, fill=blue!15, draw=blue!70!black, below=10pt of c1] (c2) {\textbf{Stretched Households}\\Kids \& tight budgets};
                \node[cbox, fill=orange!20, draw=orange!70!black, below=10pt of c2] (c3) {\textbf{Lean Savers}\\Moderate income, lower costs};
